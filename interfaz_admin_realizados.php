...
// Consulta para obtener los tickets realizados con límite y offset
$consulta_tickets_realizados = "SELECT `TicketID`, `aula`, `materia`, `Descripcion`, `Estado`, `FechaCreacion`, `categoria` FROM `tickets` WHERE `Estado` = 'Realizado' LIMIT $tickets_por_pagina OFFSET $offset";
$result_tickets_realizados = mysqli_query($connect, $consulta_tickets_realizados);

if (!$result_tickets_realizados) {
    die("Error en la consulta: " . mysqli_error($connect));
}

$consulta_total_tickets_realizados = "SELECT COUNT(*) as total FROM `tickets` WHERE `Estado` = 'Realizado'";
$result_total_tickets_realizados = mysqli_query($connect, $consulta_total_tickets_realizados);
$total_tickets_realizados = mysqli_fetch_assoc($result_total_tickets_realizados)['total'];
$total_paginas = ceil($total_tickets_realizados / $tickets_por_pagina);
?>
<!DOCTYPE html>
<html lang="es">
...
<body>
    <div class="contego">
        ...
        <div class="panel3">
            <h1 id="titulo"><i class="bi bi-check-circle-fill"></i>Realizados</h1>
            <div class="contenedor-tabla">
                ...
                <table class="blueTable">
                    ...
                    <tbody>
                        <?php
                        while ($row = mysqli_fetch_assoc($result_tickets_realizados)) {
                            echo "<tr>";
