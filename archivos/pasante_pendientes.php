...
if (!$connect) {
    ...
    $consultaPendientes = "SELECT COUNT(*) AS total FROM `tickets` WHERE `estado` = 'realizado'";
                ...
                <div class="md:mt-12 md:w-48 md:fixed md:left-0 md:top-0 content-center md:content-start text-left justify-between">
                    <ul class="list-reset flex flex-row md:flex-col pt-3 md:py-3 px-1 md:px-2 text-center md:text-left">
                        <li class="mr-3 flex-1">
                            <a href="interfaz_pasante.php" class="block py-1 md:py-3 pl-1 align-middle text-white no-underline hover:text-white border-b-2 border-gray-800 hover:border-purple-500">
                        ...
                        <li class="mr-3 flex-1">
                            <a href="pasante_pendientes.php" class="block py-1 md:py-3 pl-1 align-middle text-white no-underline hover:text-white border-b-2 border-blue-600">
                                <i class="fas fa-clock pr-0 md:pr-3 text-blue-600"></i><span class="pb-1 md:pb-0 text-xs md:text-base text-white md:text-white block md:inline-block">Pendientes</span>
                            </a>
                        </li>
                    </ul>
                </div>
...
$consultaTicketsPendientes = "SELECT * FROM `tickets` WHERE `Estado` = 'pendiente'";
$resultTicketsPendiente = mysqli_query($connect, $consultaTicketsPendientes);

if ($resultTicketsPendiente) {
    // Muestra los resultados en una tabla con líneas divisorias
    echo '<form method="post" action="pasante_pendientes.php">'; // Formulario para procesar los tickets marcados
    ...
    echo '<thead>
        <tr>
            ...
            <td class="tg-0pky">Realizado</td>
            <td class="tg-0pky">Rechazado</td>
        </tr>
    </thead>';

    while ($rowTicket = mysqli_fetch_assoc($resultTicketsPendiente)) {
        ...
        echo '<td class="text-center">' . '<input type="radio" name="estado[' . $rowTicket['TicketID'] . ']" value="realizado"> ' . '</td>';
        echo '<td class="text-center">' . '<input type="radio" name="estado[' . $rowTicket['TicketID'] . ']" value="rechazado"> ' . '</td>';
        echo '</tr>';
    }
...
?>
                ...
                <form method="post" action="pasante_realizados.php"> <!-- El formulario se envía a sí mismo -->
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['cambiarEstado'])) {
        if (isset($_POST['estado']) && is_array($_POST['estado'])) {
            foreach ($_POST['estado'] as $ticketID => $nuevoEstado) {
                // Aquí se realiza la actualización del estado en la base de datos
                $consultaActualizarEstado = "UPDATE `tickets` SET `Estado` = '$nuevoEstado' WHERE `TicketID` = $ticketID";
                $resultadoActualizacion = mysqli_query($connect, $consultaActualizarEstado);
            }
        } else {
            echo "<center>No se han seleccionado estados para actualizar. </center><br>";
        }
    }
}
?>
...
<div class="text-center mt-4">
    <form method="post" action="pasante_realizados.php"> <!-- El formulario se envía a sí mismo -->
        <button type="submit" name="cambiarEstado" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
            Cambiar Estado de Tickets
        </button> <!-- Botón para cambiar el estado -->
    </form>
</div>
</div>
</section>
